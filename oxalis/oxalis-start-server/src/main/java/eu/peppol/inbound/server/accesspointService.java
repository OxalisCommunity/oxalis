...
@WebService(serviceName = "accesspointService",
        ...
        wsdlLocation = "WEB-INF/wsdl/accesspointService/wsdl_v1.5.wsdl")
...
public class accesspointService {
    ...
    @Action(
            input = "http://www.w3.org/2009/02/ws-tra/Create",
...
/*
        ...
        CertificateFactory certFactory = CertificateFactory.getInstance(CERT_X509);
        X509Certificate metaCert = (X509Certificate) certFactory.generateCertificate(in);

        Log.debug("Metadata Certificate - Serial Number: " + metaCert.getSerialNumber());
*/
        ...
        if (isTheSameCert(metaCert, oursCert)) {
            if (isTheSame(recipientAPUrl, senderAPUrl)) {
                ...
                Log.info("This is a local request - storage directly " + metadata.getRecipientValue());
                deliverLocally(metadata, body);

            } else {

                Log.info("Sender Access Point and Receiver Access Point are different");
                Log.info("This is a request for a remote Access Point: " + recipientAPUrl);
    ...
    private boolean isTheSameCert(X509Certificate meta, X509Certificate ours) {
        return ours.getSerialNumber().toString().equals(meta.getSerialNumber().toString());
    }
